...
class TestSubscription:
    def test_successful_subscription_on_home_page(self, home_page: HomePage):
        ...
        home_page.subscribe_to_updates_component.fill_fields(email=authorized_test_user.account_information.email)
    ...
    def test_successful_subscription_on_cart_page(self, cart_page: CartPage):
        cart_page.open()
        cart_page.breadcrumb_component.check_breadcrumb_current_item(current_page_item='Shopping Cart')
        cart_page.subscribe_to_updates_component.check_title()
        cart_page.subscribe_to_updates_component.fill_fields(email=authorized_test_user.account_information.email)
